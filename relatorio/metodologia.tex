...
(deletar?)A interação do usuários com as ferramentas disponíveis está representada no diagrama da Figura \ref{fig:ucdferramentas}. O usuário pode criar arquivos \emph{raffle} e utilizá-los na aplicação de borramento e blocagem e na simulação de \emph{streaming}. Estas três ferramentas dependem de um arquivo \emph{raffle} como parâmetro de entrada. A outra ação de interação ...

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.9\textwidth]{./diagramas/ferramentas.png}
	\caption{Ferramentas.}
	\label{fig:ucdferramentas}
	\fonte{Autoria Própria}
\end{figure}
