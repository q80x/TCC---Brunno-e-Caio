...
A primeira tela trata de configurações básicas da sessão, tal como um nome e descrição para facilitar a identificação, assim como a métrica que será utilizada e o número de espectadores. Devido as limitações encontradas no \emph{hardware}, somente as métricas DSIS e SDSCE foram implementadas. As demais, DSCQS e SSCQE, que estão em modo teste no equipamento de avaliação, não foram implementadas pois a modificação do \emph{firmware} não faz parte do escopo deste projeto.
